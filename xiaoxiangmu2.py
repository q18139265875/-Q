...
jieba.load_userdict("renwubiao2.txt")#加载人物表
f2=open('renwubiao2.txt','r',encoding='utf-8').read()
with codecs.open("黎明破晓的街道.txt", 'r', 'utf8') as f:
    for line in f.readlines():
        poss = jieba.lcut(line)  # 分词，返回词性
        lineNames.append([])  # 为本段增加一个人物列表
        for w in poss:
            if  w not in f2 or len(w) < 2:
                continue  # 当分词长度小于2或该词词性不为nr（人名）时认为该词不为人名
            lineNames[-1].append(w)  # 为当前段的环境增加一个人物
            if names.get(w) is None:  # 如果某人物（w.word）不在人物字典中
                names[w] = 0
                relationships[w] = {}
            names[w] += 1
